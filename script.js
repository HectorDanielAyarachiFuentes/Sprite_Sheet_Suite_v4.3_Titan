document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- History (Undo/Redo) ---
    const saveState = () => { if(isLocked) return; historyStack = historyStack.slice(0, historyIndex + 1); historyStack.push(JSON.stringify(frames)); historyIndex++; updateHistoryButtons(); saveCurrentSession(); };
    ...
    // --- Drawing & Interaction ---
    const drawAll = () => {
        ctx.clearRect(0,0,canvas.width,canvas.height); const activeClip=getActiveClip(); const allSubFrames=getFlattenedFrames(); frames.forEach(frame=>{const isSelected=selectedFrameId===frame.id; ctx.strokeStyle=isSelected?'var(--danger)':'rgba(122, 162, 247, 0.5)'; ctx.lineWidth=isSelected?2:1; ctx.setLineDash(frame.type==='group'?[4,4]:[]); ctx.strokeRect(frame.rect.x,frame.rect.y,frame.rect.w,frame.rect.h); ctx.setLineDash([]); if(isSelected && !isLocked)drawResizeHandles(frame.rect); if(frame.type==='group'){
            const sliceColor = '#f7768e';
            const overrideColor = '#e0af68';
            ctx.lineWidth=1; 
            
            ctx.strokeStyle = sliceColor;
            frame.hSlices.forEach(sliceY=>{ctx.beginPath(); ctx.moveTo(frame.rect.x,frame.rect.y+sliceY); ctx.lineTo(frame.rect.x+frame.rect.w,frame.rect.y+sliceY); ctx.stroke()}); 
            
            const yCoords=[0,...frame.hSlices,frame.rect.h]; 
            for(let i=0; i<yCoords.length-1; i++){const rowYStart=frame.rect.y+yCoords[i],rowYEnd=frame.rect.y+yCoords[i+1]; 
            frame.vSlices.forEach(slice=>{const xPos=slice.rowOverrides[i]!==undefined?slice.rowOverrides[i]:slice.globalX; 
            if(xPos===null)return; 
            const isOverridden=slice.rowOverrides[i]!==undefined; 
            ctx.strokeStyle=isOverridden? overrideColor : sliceColor; ctx.beginPath(); ctx.moveTo(frame.rect.x+xPos,rowYStart); ctx.lineTo(frame.rect.x+xPos,rowYEnd); ctx.stroke()})}}}); 
            
            allSubFrames.forEach(subFrame=>{const isIncluded=activeClip?.frameIds.includes(subFrame.id); ctx.fillStyle=isIncluded?'rgba(122, 162, 247, 0.15)':'rgba(30,30,45,0.4)'; ctx.fillRect(subFrame.rect.x,subFrame.rect.y,subFrame.rect.w,subFrame.rect.h); ctx.fillStyle=isIncluded?'rgba(255,255,255,0.8)':'rgba(169,177,214,0.6)'; ctx.font='12px var(--font-sans)'; ctx.fillText(subFrame.id,subFrame.rect.x+4,subFrame.rect.y+14)}); if(isDrawing&&newRect){ctx.strokeStyle='var(--warning)'; ctx.strokeRect(newRect.x,newRect.y,newRect.w,newRect.h)}drawRulers()};
    ...
    canvas.addEventListener('mousedown', (e) => { startPos = getMousePos(e); resizeHandle = getHandleAtPos(startPos); draggedSlice = getSliceAtPos(startPos); const frameAtPos = getFrameAtPos(startPos); if (resizeHandle) { isResizing = true; } else if (draggedSlice) { isDraggingSlice = true; } else if (frameAtPos) { if(!isLocked) isDragging = true; selectedFrameId = frameAtPos.id; } else { selectedFrameId = null; if(!isLocked) {isDrawing = true; newRect = { x: startPos.x, y: startPos.y, w: 0, h: 0 };} } drawAll(); updateSlicerUI(); });
    canvas.addEventListener('mousemove', (e) => { const pos = getMousePos(e); const handle = getHandleAtPos(pos); const slice = getSliceAtPos(pos); if (handle) { canvas.style.cursor = 'move'; if(!isLocked) { if (handle === 'tl' || handle === 'br') canvas.style.cursor = 'nwse-resize'; else if (handle === 'tr' || handle === 'bl') canvas.style.cursor = 'nesw-resize'; else if (handle === 't' || handle === 'b') canvas.style.cursor = 'ns-resize'; else if (handle === 'l' || handle === 'r') canvas.style.cursor = 'ew-resize';}} else if (slice) { canvas.style.cursor = slice.axis === 'v' ? 'ew-resize' : 'ns-resize'; } else if (getFrameAtPos(pos)) { canvas.style.cursor = isLocked ? 'default' : 'move'; } else { canvas.style.cursor = isLocked ? 'default' : 'crosshair'; } const frame = frames.find(f => f.id === selectedFrameId); if (isResizing && frame && !isLocked) { let { x, y, w, h } = frame.rect; const ox2 = x + w, oy2 = y + h; if (resizeHandle.includes('l')) x = pos.x; if (resizeHandle.includes('t')) y = pos.y; if (resizeHandle.includes('r')) w = pos.x - x; if (resizeHandle.includes('b')) h = pos.y - y; if (resizeHandle.includes('l')) w = ox2 - x; if (resizeHandle.includes('t')) h = oy2 - y; frame.rect = { x, y, w, h }; } else if (isDragging && frame && !isLocked) { const dx = pos.x - startPos.x, dy = pos.y - startPos.y; frame.rect.x += dx; frame.rect.y += dy; startPos = pos; } else if (isDraggingSlice && frame) { if (draggedSlice.axis === 'v') { let newX = pos.x - frame.rect.x; if (newX < 0) newX = 0; if (newX > frame.rect.w) newX = frame.rect.w; const vSlice = frame.vSlices[draggedSlice.index]; if (e.altKey) vSlice.rowOverrides[draggedSlice.rowIndex] = newX; else vSlice.globalX = newX; } else { let newY = pos.y - frame.rect.y; if (newY < 0) newY = 0; if (newY > frame.rect.h) newY = frame.rect.h; frame.hSlices[draggedSlice.index] = newY; } } else if (isDrawing && !isLocked) { newRect.w = pos.x - newRect.x; newRect.h = pos.y - newRect.y; } drawAll(); });
    canvas.addEventListener('mouseup', () => { if (isResizing || isDragging || isDraggingSlice) { const frame = frames.find(f => f.id === selectedFrameId); if (frame && frame.rect.w < 0) { frame.rect.x += frame.rect.w; frame.rect.w *= -1; } if (frame && frame.rect.h < 0) { frame.rect.y += frame.rect.h; frame.rect.h *= -1; } updateAll(); } else if (isDrawing && newRect) { if (newRect.w < 0) { newRect.x += newRect.w; newRect.w *= -1; } if (newRect.h < 0) { newRect.y += newRect.h; newRect.h *= -1; } if (newRect.w > 4 && newRect.h > 4) { const newId = frames.length > 0 ? Math.max(...frames.map(f => f.id)) + 1 : 0; frames.push({ id: newId, name: `frame_${newId}`, rect: newRect, type: 'simple' }); selectedFrameId = newId; updateAll(); } else { drawAll(); updateSlicerUI(); } } isDrawing = isDragging = isResizing = isDraggingSlice = false; newRect = resizeHandle = draggedSlice = null; });
    ...
    clearButton.addEventListener('click', () => { if(isLocked){ showToast('Desbloquea los frames primero (L)', 'warning'); return; } if(confirm('¿Seguro que quieres borrar todos los frames?')) { clearAll(); } });
    ...
    const createNewClip = (name) => { const newName = name || prompt("Nombre del nuevo clip:", `Clip ${clips.length + 1}`); if (!newName) return; clips.push({ id: Date.now(), name: newName, frameIds: [] }); activeClipId = clips[clips.length - 1].id; updateUI(); resetAnimation(); };
    ...
    deleteClipButton.addEventListener('click', () => { if (clips.length <= 1) return showToast("No puedes eliminar el último clip.", 'warning'); if(confirm('¿Eliminar clip?')) { clips = clips.filter(c => c.id !== activeClipId); activeClipId = clips[0]?.id || null; updateUI(); resetAnimation();} });
    ...
    framesList.addEventListener('change', (e) => { if(e.target.matches('[data-frame-id]')) { const clip = getActiveClip(); if(!clip) return; const id = parseInt(e.target.dataset.frameId); if (e.target.checked) { if(!clip.frameIds.includes(id)) clip.frameIds.push(id); } else { clip.frameIds = clip.frameIds.filter(fid => fid !== id); } updateAll(false); }});
    ...
    // --- LÓGICA CORREGIDA: getAnimationFrames ahora depende del clip activo ---
    const getAnimationFrames = () => {
        const clip = getActiveClip();
        if (!clip) return [];
        const all = getFlattenedFrames();
    ...
    const toggleLock = () => {
        isLocked = !isLocked;
        lockFramesButton.textContent = isLocked ? '🔒' : '🔓';
        lockFramesButton.classList.toggle('locked', isLocked);
        showToast(isLocked ? 'Frame principal bloqueado' : 'Frame principal desbloqueado', 'primary');
        drawAll();
    };
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if ((e.key === 'Delete' || e.key === 'Backspace') && selectedFrameId !== null) { if(isLocked) { showToast('Desbloquea el frame para eliminar (L)', 'warning'); return; } e.preventDefault(); frames = frames.filter(f => f.id !== selectedFrameId); selectedFrameId = null; updateAll(); }
